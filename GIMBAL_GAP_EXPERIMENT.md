...
5. 转身后低头，普通绿色巡线检测再次确认方向与居中后才交还控制权。如果曲线暂时不满足普通检测的近、远采样要求，实验任务可利用画面下方连续可见的蓝线作低速横向居中和轻微前进/转向，继续等待普通检测恢复；这个近线辅助**不能单独宣告完成或绕过巡线接口**。无近线或 3 秒内仍不能交接则停车并报告失败，不自动重试。
