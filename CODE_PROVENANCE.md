...
| INTERFACE A — MODULE REQUEST | 本分支，孙宇鹏职责 | `number_marker.py::EvidenceRequest`、`_make_evidence_request()`、`take_evidence_request()`、`acknowledge_evidence()` | 项目内个人实现 | 默认 Team 03、动态 marker ID、唯一有界 request ID、全帧图像副本及坐标、得分触发与回执后的本模块状态；删除旧的模块内 OpenCV renderer。 | 向正式证据层交付准确请求，只有成功写盘回执才加入 `saved_ids`；失败即时清理本模块瞬态。 |
