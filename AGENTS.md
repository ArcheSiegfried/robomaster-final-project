...
- Treat `models.py`, `camera_source.py`, `runtime.py`, `motion_output.py`, `gimbal_output.py`, and `main.py` as high-conflict public files. A change requires prior coordination by the integration lead, an interface-version decision, and integration tests.
...
- Task code returns `VisualDetection`, `TaskUpdate`, optional `MotionCommand`, and optional `GimbalCommand`. It must never call the RoboMaster SDK or bypass the centralized chassis or gimbal outlets.
