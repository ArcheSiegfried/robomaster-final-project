...
1. 丢弃过期请求（`ttl is None` 视为永不过期）与**冷却中**（熔断）模块的请求；
...
**熔断（circuit breaker）**：每个模块一次运行最长 `RUN_SECONDS`（8s，协调器侧计时，
不含被冻结的时段）；结束一次运行后 `COOLDOWN_SECONDS`（5s）内不许再接管
（`lock_out` / `locked_out`，由 `select()` 直接丢弃请求，协调器也不会去 step 它）。
"""
...
#: 熔断参数（集成负责人的要求，**对每个模块一视同仁，没有豁免**）：
#:   一次运行最长 `RUN_SECONDS` 秒；结束一次运行后 `COOLDOWN_SECONDS` 秒内不许再接管。
#: 2026-09-18 实车测试定稿（集成负责人）：一次最多 **6 秒**、跑完冷却 **3 秒**。
RUN_SECONDS = 6.0
...
class ControlArbiter:
    def __init__(
        ...
        #: 熔断冷却：模块"结束一次运行"后这么多秒内不许再接管（0 = 关闭）。
        self.cooldown_seconds = float(cooldown_seconds)
        #: 模块名 -> 冷却截止时刻（monotonic）。
        self._locked_until = {}

    # -- 熔断冷却 --------------------------------------------------------
    def lock_out(self, module: str, now: float, seconds: Optional[float] = None) -> None:
        """让一个模块进入冷却（它"结束一次运行"时调用）。"""
        span = self.cooldown_seconds if seconds is None else float(seconds)
        if not module or span <= 0.0:
            return
        self._locked_until[module] = now + span

    def locked_out(self, module: str, now: float) -> float:
        """模块还剩多少秒处于冷却；0 = 没被锁（顺带清掉已经过期的记录）。"""
        until = self._locked_until.get(module)
        if until is None:
            return 0.0
        remaining = until - now
        if remaining <= 0.0:
            del self._locked_until[module]
            return 0.0
        return remaining
    ...
    def select(
        ...
        valid = [
            ...
            if not item.expired(now) and self.locked_out(item.module, now) <= 0.0
        ]
