...
from control_arbiter import COOLDOWN_SECONDS, RUN_SECONDS  # noqa: E402
...
class BreakerTests(ArbitrationCase):
    """熔断：每个模块一次最多跑 RUN_SECONDS，跑完冷却 COOLDOWN_SECONDS（对每个模块一视同仁，无豁免）。"""

    def test_breaker_numbers_are_the_agreed_values(self):
        """这两条数值是实车测试后定下来的，故意钉住：改动必须是有意识的。"""
        self.assertEqual(RUN_SECONDS, 6.0)
        self.assertEqual(COOLDOWN_SECONDS, 3.0)

    def test_one_run_is_cut_at_the_breaker_limit(self):
        obstacle = StubTask("obstacle", [RUN], 70)
        harness = self.make_harness((obstacle,))
        decisions = []
        for _ in range(40):                      # 每帧 0.25s，足够越过上限
            decisions.append(self.feed(harness, step=0.25))
            if any("breaker run limit" in " ".join(d.errors) for d in decisions):
                break

        cut = [d for d in decisions if "breaker run limit" in " ".join(d.errors)]
        self.assertEqual(len(cut), 1, "第一次到点就该被切")
        self.assertTrue(cut[0].force_stop, "熔断必须硬停")
        self.assertEqual(obstacle.resets, 1, "熔断释放要 reset 掉那个模块")
        self.assertIn("%.1fs" % RUN_SECONDS, " ".join(cut[0].errors))

    def test_a_finished_module_cannot_run_again_during_the_cooldown(self):
        obstacle = StubTask("obstacle", [RUN, DONE] + [RUN] * 200, 70)
        harness = self.make_harness((obstacle,))
        self.feed(harness)                       # 接管
        harness.now += 0.5
        release = self.feed(harness, step=0.0)   # 它自己结束了

        self.assertIn("locked out", release.owner_change or "", "结束时要打熔断日志")

        # 冷却期内（每帧 0.5s，总共走不到 COOLDOWN_SECONDS）
        frames = max(1, int(COOLDOWN_SECONDS / 0.5) - 2)
        for _ in range(frames):
            self.feed(harness, step=0.5)
        calls = obstacle.calls
        decision = self.feed(harness, step=0.5)

        self.assertIsNone(decision.task_name, "冷却期内不许再接管")
        self.assertEqual(obstacle.calls, calls, "冷却期内连 step 都不该被调用")

    def test_after_the_cooldown_it_may_take_over_again(self):
        obstacle = StubTask("obstacle", [RUN, DONE] + [RUN] * 200, 70)
        harness = self.make_harness((obstacle,))
        self.feed(harness)
        harness.now += 0.5
        ...
        owner = None
        for _ in range(30):                      # 每帧 0.5s，越过冷却
            owner = self.feed(harness, step=0.5)
            if owner.task_name:
                break
        self.assertEqual(owner.task_name, "obstacle", "冷却结束后应该能重新接管")

    def test_a_finished_module_gives_others_a_turn_while_cooling(self):
        obstacle = StubTask("obstacle", [RUN, DONE], 70)
        route = StubTask("route", [IDLE] + [RUN] * 200, 60)
        harness = self.make_harness((obstacle, route))
        self.feed(harness)                       # obstacle 先接管
        self.assertEqual(harness.coordinator.active_task_name, "obstacle")
        harness.now += 0.5
        self.feed(harness, step=0.0)             # obstacle 结束 -> 进入冷却
        ...
        for _ in range(30):
            ...
            if owner.task_name:
                break
        self.assertEqual(owner.task_name, "route", "冷却只锁刚跑完的那个，别的模块照常可以接管")

    def test_frozen_time_does_not_count_toward_the_run_limit(self):
        obstacle = StubTask("obstacle", [RUN], 70)
        light = StubTask("traffic_light", [IDLE] + [STOP] * 12 + [IDLE] * 200, 90)
        harness = self.make_harness((obstacle, light))
        self.feed(harness)                       # obstacle 接管

        for _ in range(12):                      # 6 秒红灯：远超运行上限，但那是冻结时间
            self.feed(harness, step=0.5)

        decision = None
        for _ in range(2):                       # 绿灯后只让它再跑 1 秒
            decision = self.feed(harness, step=0.5)

        self.assertEqual(decision.task_name, "obstacle", "等红灯的时间不该把它熔断")
        self.assertFalse(
            any("breaker run limit" in " ".join(d.errors) for d in harness.traces[-2:])
        )

    def test_the_red_light_itself_is_also_cut_at_the_breaker_limit(self):
        """按集成负责人的字面要求：**每个模块**都熔断，红绿灯也不例外。"""
        light = StubTask("traffic_light", [STOP], 90)
        harness = self.make_harness((light,))
        decisions = [self.feed(harness, step=0.5) for _ in range(20)]

        self.assertTrue(
            any("breaker run limit" in " ".join(d.errors) for d in decisions),
            "红绿灯也要在上限被切（无豁免）",
        )
        locked = [d for d in decisions if "locked out" in (d.owner_change or "")]
        self.assertTrue(locked, "切掉之后红绿灯要进入 5 秒冷却")
