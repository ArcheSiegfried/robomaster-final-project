...
- 尚未完成：阶段 3～5、完整实车联调和 PR。`RoboMaster_期末项目规则书.md` 是用户放在项目根目录的未跟踪文件，当前保留原样、不混入代码提交。
- 下一步准确操作：检查 `green_junction.py` 内置认灯及选路规则，将生产默认固定为“单侧绿灯、走绿灯侧”，补定向用例与配置说明；再审查全任务互斥和释放路径。
